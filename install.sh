...
git archive --prefix=mutdetect_$MUTDETECT_VERSION$PREFIX/ $MUTDETECT_VERSION 2>$ERROR_LOG | (cd $MUTDETECT_VERSION_DIR && tar xf -) 2>$ERROR_LOG
rtrn=$?
if [[ rtrn -ne 0 ]]; then
...
#cp -R * $PREFIX/. 2>&1
cd /usr/local/archives/mutdetect/mutdetect_$MUTDETECT_VERSION/ 2>$ERROR_LOG
rm -rf test 2>&$ERROR_LOG
for f in $(find . -name \* -print 2>$ERROR_LOG); do
    echo -e "cp $f in $PREFIX/$f" | tee -a $ERROR_LOG 2>&1
    cp --parents $f $PREFIX/ 2>$ERROR_LOG
done
